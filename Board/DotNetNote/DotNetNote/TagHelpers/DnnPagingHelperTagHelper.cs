  ...
  public class DnnPagingHelperTagHelper : TagHelper
  {
    ...
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
      ...
      if (PageIndex > 10)
      {
        if (!SearchMode)
        {
          strPage += "<li><a href=\"" + Url + "?Page=" + Convert.ToString(((PageIndex - 1) / (int)10) * 10)
            + "\">◀</a></li>";
        }
        else
        {
        ...
        i <= ((((PageIndex - 1) / (int)10) + 1) * 10); i++)
      {
        ...
        else
        {
          ...
          {
            strPage += "<li><a href=\"" + Url + "?Page=" + i.ToString()
            + "&SearchField=" + SearchField
              + "&SearchQuery=" + SearchQuery
              + "\">" + i.ToString() + "</a></li>"; ;
          }
      ...
      if (i < PageCount)
      {
        if (!SearchMode)
        {
          strPage += "<li><a href=\"" + Url + "?Page=" + Convert.ToString(((PageIndex - 1) / (int)10) * 10 + 11)
            + "\">▶</a></li>";
        }
        else
        {
